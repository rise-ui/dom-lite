...
where
  T: TGenericEvent,
{
  pub(crate) fn value(&self) -> &DOMNode<T> {
    self.raw.try_value().expect("Node deallocated")
  }

  pub(crate) fn value_mut(&mut self) -> &mut DOMNode<T> {
    self.raw.try_value_mut().expect("Node deallocated")
  }

  pub(crate) fn into_value(self) -> &'a mut DOMNode<T> {
  ...
  T: TGenericEvent,
{
  ...
  pub fn build_layout(&mut self) {
    ...
    while let Some(child_id) = next_child_id {
      {
        let mut _child_ref = self.get_mut(child_id);
      }
