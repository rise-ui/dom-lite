...
where
  T: TGenericEvent,
{
  pub(crate) layout_node: LayoutNode,
  pub(crate) data: DOMData<T>,
...
where
  T: TGenericEvent,
{
  pub(crate) fn append_to_layout_node(&mut self, parent: &mut DOMNode<T>) {
  ...
  pub(crate) fn remove_from_layout_node(&mut self, parent: &mut DOMNode<T>) {
    let parent = &mut parent.layout_node;
    let child = &mut self.layout_node;
  ...
  T: TGenericEvent,
{
  ...
  pub(crate) fn is_shadow_host(&self) -> Option<&DOMTree<T>> {
    match self {
  ...
  pub(crate) fn is_normal(&self) -> Option<&DOMNormalNode<T>> {
    match self {
  ...
  pub(crate) fn is_known(&self, name: KnownElementName) -> Option<&DOMNormalNode<T>> {
    match self {
  ...
  pub(crate) fn drop_event_listeners(&mut self) -> Option<&mut Vec<DOMAttribute<T>>> {
  ...
  pub(crate) fn attributes_ref(&self) -> Option<&Vec<DOMAttribute<T>>> {
        match self {
            &DOMData::Void | &DOMData::ShadowHost(_) | &DOMData::Text(_) => None,
  ...
  #[cfg_attr(rustfmt, rustfmt_skip)]
  pub(crate) fn attributes_mut(&mut self) -> Option<&mut Vec<DOMAttribute<T>>> {
        match self {
  ...
  pub(crate) fn attributes_slice(&self) -> &[DOMAttribute<T>] {
    self.attributes_ref().map(|v| &v[..]).unwrap_or(&[])
  }

  pub(crate) fn attributes_slice_mut(&mut self) -> &mut [DOMAttribute<T>] {
